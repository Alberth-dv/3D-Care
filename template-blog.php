...
$page_id = get_the_ID();
// Get Page metadata
$hide_header 		= esc_attr( get_post_meta( $page_id, 'quadro_blog_header_hide', true ) );
...
?>
	...
	<div id="primary" class="modular-wrapper">
		...
		<div id="content" class="modular-modules <?php echo $hide_header_class; ?>" role="main">
			...
			<?php // Query for the selected Blog Modules
			$args = array(
				'post_type' =>  'quadro_mods',
				'posts_per_page' => -1,
			);
			// Bring picked blog modules if there are some
			$args = quadro_add_selected_posts( get_the_ID(), 'quadro_mod_blog_modules', $args );
			$blog_mods = new WP_Query( $args );
			?>
			
			<?php if( $blog_mods->have_posts() ) : while( $blog_mods->have_posts() ) : $blog_mods->the_post(); ?>

				<?php // We already know is a blog, so we can call the template for it
